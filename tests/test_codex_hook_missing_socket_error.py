#!/usr/bin/env python3
"""
Regression test: codex-hook stop surfaces a clear socket-connect error when target socket is missing.
"""
...
def main() -> int:
    ...
    proc = subprocess.run(
        [cli_path, "--socket", missing_socket, "codex-hook", "stop"],
        input="{}",
        text=True,
        capture_output=True,
        env=env,
    ...
    print("PASS: codex-hook stop missing-socket error is explicit")
    return 0
