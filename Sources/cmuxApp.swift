...
enum MainWindowAutosaveFrameSanitizer {
    ...
    static func invalidFrameKeys(
        in dictionary: [String: Any],
        screenVisibleFrames: [CGRect]
    ) -> [String] {
        invalidFrameRecords(in: dictionary, screenVisibleFrames: screenVisibleFrames).map(\.key)
    }
    ...
    static func isMainWindowAutosaveKey(_ key: String) -> Bool {
        key.hasPrefix(keyPrefix)
            && key.contains("cmux.ContentView")
            && key.contains("-AppWindow-")
    }

    static func shouldResetAutosavedFrame(
        _ value: Any,
        screenVisibleFrames: [CGRect]
    ) -> Bool {
    ...
    static func parseAutosavedFrame(_ encoded: String) -> (windowFrame: CGRect, screenFrame: CGRect?)? {
        let values = encoded
...
struct cmuxApp: App {
    ...
    init() {
        ...
        // SwiftUI persists main-window geometry under generic synthesized
        // keys in UserDefaults. If a prior build stored a pathological size,
        // AppKit can crash during startup constraints before our own restore
        // path runs. Drop only the clearly invalid cmux main-window entries.
