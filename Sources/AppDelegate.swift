...
private func commandDigit(from event: NSEvent) -> Int? {
...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation {
    ...
    private func handleCustomShortcut(event: NSEvent) -> Bool {
        ...
        if let digit = commandDigit(from: event) {
            if let manager = tabManager,
               let targetIndex = WorkspaceShortcutMapper.workspaceIndex(forCommandDigit: digit, workspaceCount: manager.tabs.count) {
#if DEBUG
                dlog(
