...
class TerminalController {
    ...
    private func helpText() -> String {
        ...
        Notification commands:
          ...
          clear_active_ai_session <agent> [session_id] [--tab=X] [--surface=Y] - Clear tracked live AI session for a terminal panel
    ...
    /// Usage: clear_active_ai_session <agent> [session_id] [--tab=<id>] [--surface=<id>]
    private func clearActiveAISession(_ args: String) -> String {
        let parsed = parseOptions(args)
        guard let agentType = parseAIAgentType(parsed.positional.first) else {
            return "ERROR: Usage: clear_active_ai_session <agent> [session_id] [--tab=<id>] [--surface=<id>]"
        }
        let sessionId = parsed.positional.count > 1 ? normalizedOptionValue(parsed.positional[1]) : nil
        ...
        return schedulePanelMetadataMutation(
            args: args,
            options: parsed.options,
            missingPanelUsage: "clear_active_ai_session <agent> [session_id] [--tab=<id>] [--surface=<id>]"
        ) { tab, panelId in
            tab.clearActiveAISession(panelId: panelId, agentType: agentType, sessionId: sessionId)
        }
    }
    ...
    private func prefillSessionResume(_ args: String) -> String {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if let activeSnapshot = tab.activeAISession(panelId: panelId, agentType: agentType),
               let activeSessionId = activeSnapshot.sessionId,
